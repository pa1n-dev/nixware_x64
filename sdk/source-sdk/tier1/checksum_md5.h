...
inline static void md5_transform(unsigned int buf[4], unsigned int const in[16])
{
...
inline void md5_final(unsigned char digest[MD5_DIGEST_LENGTH], md5_context_t* ctx)
{
	unsigned count;
	unsigned char* p;
...
inline char* md5_print(unsigned char* hash, int hash_len)
{
	static char sz_return[64];

	Q_BINARYTOHEX(hash, hash_len, sz_return, sizeof(sz_return));
	return sz_return;
}
