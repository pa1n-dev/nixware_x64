...
void ImGui::HotKey(const char* label, int* value)
{
    ...
    window->DC.CursorPos.y -= label_size.y + style.ItemSpacing.y + style.FramePadding.y * 2;
    ...
    bool changed = false;
    if (int key = *value; g.ActiveId == id)
    {
    ...
    char buff[64] = { };
    sprintf_s(buff, sizeof(buff), "%s", *value != 0 && g.ActiveId != id ? KeyNames[*value] : g.ActiveId == id ? "WAIT KEY" : "NONE");
    const ImVec2 buff_size = CalcTextSize(buff, NULL, true);

    RenderText(ImVec2(total_bb.Max.x - (buff_size.x + 10.f), total_bb.Min.y + (square_sz / 2) - (buff_size.y / 2)), buff);
}
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(total_bb.Min.x, total_bb.Min.y + (square_sz / 2) - (label_size.y / 2)), label);

    if (value_size.x > 0.0f)
        RenderText(ImVec2(total_bb.Max.x - (value_size.x + 10.f), total_bb.Min.y + (square_sz / 2) - (value_size.y / 2)), value_text_begin, value_text_end);
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    {
        g.ComboPreviewData.PreviewRect = total_bb;
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderText(ImVec2(frame.Min.x + style.ItemSpacing.x, frame.GetCenter().y - label_size.y / 2), preview_value);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w - 10.f, label_size.y + style.FramePadding.y + style.ItemInnerSpacing.y * 3.f));
    ...
    if (label_size.x > 0.0f)
    {
        ...
        RenderText(ImVec2(total_bb.Max.x - CalcTextSize(value_buf).x, total_bb.Min.y), value_buf);
    }

    ImRect grab_bb;
    const bool value_changed = SliderBehavior(ImRect(frame_bb.Min - ImVec2(13.f, 0.f), frame_bb.Max + ImVec2(7.f, 0.f)), id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImVec2 total_size = ImVec2(frame_size.x, frame_size.y + (label_size.x > 0.0f ? style.ItemInnerSpacing.y * 2 + label_size.y : 0.0f));

    ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + frame_size);
    const ImRect total_bb(frame_bb.Min, frame_bb.Min + total_size);

    if (label_size.x > 0)
        frame_bb = ImRect(ImVec2(frame_bb.Min.x, frame_bb.Max.y), total_bb.Max);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = GetColorU32(ImGuiCol_FrameBgHovered, render_cursor ? 1.0f : 0.6f); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    const ImVec2 backup_pos = window->DC.CursorPos;
    window->DC.CursorPos.y -= CalcTextSize(label).y + style.ItemSpacing.y + style.FramePadding.y * 2;

    ImGuiWindow* picker_active_window = NULL;
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ImVec2 preview_size = ImVec2(20.f, 10.f);
        window->DC.CursorPos.x += ImGui::GetColumnWidth() - (preview_size.x + 10.f);

        const ImVec4 col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.0f);
        if (ColorButton("##ColorButton", col_v4, flags, preview_size))
        {
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                SetNextItemWidth(200.f); // Use 256 + bar sizes?
                value_changed |= ColorPicker4("##picker", col, ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_DisplayHex | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_NoSmallPreview, &g.ColorPickerRef.x);
            }
            EndPopup();
        }
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        RenderFrame(ImVec2(bar0_pos_x - 2.f, bar0_line_y), ImVec2(bar0_pos_x + bars_width + 2.f, bar0_line_y + 2.f), ImColor(ImVec4(1.f, 1.f, 1.f, 0.7f)), false);
    ...
    draw_list->AddLine(ImVec2(sv_cursor_pos.x - 7.f, sv_cursor_pos.y), ImVec2(sv_cursor_pos.x - 2, sv_cursor_pos.y), ImColor(ImVec4(1.f, 1.f, 1.f, 0.7f)));
    draw_list->AddLine(ImVec2(sv_cursor_pos.x + 7.f, sv_cursor_pos.y), ImVec2(sv_cursor_pos.x + 2, sv_cursor_pos.y), ImColor(ImVec4(1.f, 1.f, 1.f, 0.7f)));
    draw_list->AddLine(ImVec2(sv_cursor_pos.x, sv_cursor_pos.y + 7.f), ImVec2(sv_cursor_pos.x, sv_cursor_pos.y + 2), ImColor(ImVec4(1.f, 1.f, 1.f, 0.7f)));
    draw_list->AddLine(ImVec2(sv_cursor_pos.x, sv_cursor_pos.y - 7.f), ImVec2(sv_cursor_pos.x, sv_cursor_pos.y - 2), ImColor(ImVec4(1.f, 1.f, 1.f, 0.7f)));
    ...
    if (alpha_bar)
    {
        ...
        RenderFrame(ImVec2(bar1_pos_x - 2.f, bar1_line_y), ImVec2(bar1_pos_x + bars_width + 2.f, bar1_line_y + 2.f), ImColor(ImVec4(1.f, 1.f, 1.f, 0.7f)), false);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImVec2 size(size_arg.x != 0.0f ? size_arg.x : GetColumnWidth(), size_arg.y != 0.0f ? size_arg.y : label_size.y);
    ImVec2 pos = window->DC.CursorPos;
    pos.y += window->DC.CurrLineTextBaseOffset;
    ItemSize(size, 0.0f);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    ImVec2 size = ImVec2(GetWindowWidth() / tab_bar->Tabs.Size, 35.f);
