...
namespace ImGui
{
    ...
    // Widgets: Text
    IMGUI_API void          HotKey(const char* label, int* value);
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_DelayBig                      = 1 << 11,  // Return true after io.HoverDelayNormal elapsed (~3.5 sec)
    ImGuiHoveredFlags_DelayNormal                   = 1 << 12,  // Return true after io.HoverDelayNormal elapsed (~0.30 sec)
    ImGuiHoveredFlags_DelayShort                    = 1 << 13,  // Return true after io.HoverDelayShort elapsed (~0.10 sec)
    ImGuiHoveredFlags_NoSharedDelay                 = 1 << 14,  // Disable shared delay system where moving from one item to the next keeps the previous timer for a short time (standard for tooltips with long delays)
};
...
struct ImGuiIO
{
    ...
    float       HoverDelayBig;                  // = 5 sec       // Delay on hovering before IsItemHovered(ImGuiHoveredFlags_DelayNormal) returns true.
